...
class ReceiptPdfService extends AbstractFPDIService
{
    ...
    public function __construct($app)
    {
        ...
        $this->labelCellByFront[] = '金額(内税)';
        $this->labelCellByFront[] = '消費税額';        
        $this->widthCellByFront = array(17, 108, 8, 16, 19, 15);
    ...
    protected function renderOrderDataDetail(Order $Order, $addressNameFlg)
    {
        ...
        // 総合計金額
        $this->SetFont(self::FONT_SJIS, 'B', 11);
        $paymentTotalText = number_format($Order->getPaymentTotal()).' '.self::MONETARY_UNIT;

        $this->setBasePosition(156, 60);
        $this->Cell(30, 6, '', 0, 0, '', 0, '');
        $this->Cell(70, 6, "合計金額：￥ ".$paymentTotalText, 0, 2, 'R', 0, '');
        ...
        // お支払い方法
        $this->Cell(70, 6, "お支払い方法：".$Order->getPaymentMethod(), 0, 2, 'R', 0, '');
    ...
    protected function renderOrderDataByFront(Order $Order)
    {
        ...
        foreach ($Order->getOrderDetails() as $OrderDetail) {
            ...
            // 小計（商品毎）
            $arrOrder[$i][4] = $this->getPriceFilter($OrderDetail->getTotalPrice());
            $arrOrder[$i][5] = $this->getPriceFilter($tax);
            $arrOrder[$i][6] = $OrderDetail->getPrice();

            ++$i;
        }
        $this->SetFont(self::FONT_SJIS, '', 7);
        $rowIndex = 0;
        $price10 = 0;
        ...
        $this->Cell(($this->widthCellByFront[0]+$this->widthCellByFront[1]), 7, '', 1, 0, 'C', 0);
        $this->Cell(($this->widthCellByFront[2]+$this->widthCellByFront[3]+$this->widthCellByFront[4]), 7, '10%対象税抜金額', 1, 0, 'C', 0); 
        $this->Cell($this->widthCellByFront[5], 7, $price10, 1, 0, 'R', 0);
        $this->Ln();

        $this->Cell(($this->widthCellByFront[0]+$this->widthCellByFront[1]), 7, '', 1, 0, 'C', 0);
        $this->Cell(($this->widthCellByFront[2]+$this->widthCellByFront[3]+$this->widthCellByFront[4]), 7, '8%対象税抜金額', 1, 0, 'C', 0); 
        $this->Cell($this->widthCellByFront[5], 7, $price8, 1, 0, 'R', 0);
        $this->Ln();

        $taxSum = $this->getTaxTotalByTaxRate($Order);        
        $this->Cell(($this->widthCellByFront[0]+$this->widthCellByFront[1]), 7, '', 1, 0, 'C', 0);
        $this->Cell(($this->widthCellByFront[2]+$this->widthCellByFront[3]+$this->widthCellByFront[4]), 7, '10%対象消費税', 1, 0, 'C', 0);        
        if (isset($taxSum[10]))
        {
            // 消費税（10％）
            $tax = $this->getPriceFilter($taxSum[10]);
            $this->Cell($this->widthCellByFront[5], 7, $tax, 1, 0, 'R', 0);
        }else {
        ...
        $this->Cell(($this->widthCellByFront[0]+$this->widthCellByFront[1]), 7, '', 1, 0, 'C', 0);
        $this->Cell(($this->widthCellByFront[2]+$this->widthCellByFront[3]+$this->widthCellByFront[4]), 7, '8%対象消費税', 1, 0, 'C', 0);
        if (isset($taxSum[8]))
        {
            // 消費税（8％）
            $tax = $this->getPriceFilter($taxSum[8]);
            $this->Cell($this->widthCellByFront[5], 7, $tax, 1, 0, 'R', 0);
        }else {
            $this->Cell($this->widthCellByFront[5], 7, $this->getPriceFilter(0), 1, 0, 'R', 0);
        }
