...
class InstallController
{
    ...
    public function step4(InstallApplication $app, Request $request)
    {
        ...
        if (empty($sessionData['database']) && empty($sessionData['database_host']) &&
            empty($sessionData['database_port']) && empty($sessionData['database_name']) &&
            empty($sessionData['database_user'])) {
            $form->get('database')->setData('pdo_mysql');
            $form->get('database_host')->setData('mysql017.phy.heteml.lan');
            $form->get('database_port')->setData('3306');
            $form->get('database_name')->setData('_ryosuke0_no9ugx');
            $form->get('database_user')->setData('_ryosuke0_no9ugx');
        }
    ...
    public function step5(InstallApplication $app, Request $request)
    {
        ...
        if ($this->isValid($request, $form)) {
            ...
            if (isset($sessionData['agree']) && $sessionData['agree'] == '1') {
                ...
                $basePath = str_replace("/html", "", $request->getBasePath());
                $params = array(
