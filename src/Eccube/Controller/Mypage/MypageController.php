...
class MypageController extends AbstractController
{
	...
	public function exportReceipt(Application $app, Request $request, $id) {
        // サービスの取得
        /* @var ReceiptPdfService $service */
        $service = $app['ReceiptPdf.service.receipt_pdf'];
        // 購入情報からPDFを作成する
        $status = $service->makePdfFromFront($id);
        
        if ($status) {    
            // ダウンロードする
            $response = new Response(
                $service->outputPdfFromFront(),
                200,
     ...
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function video(Application $app, Request $request, $id)
    {
        /* @var $softDeleteFilter \Eccube\Doctrine\Filter\SoftDeleteFilter */
        $softDeleteFilter = $app['orm.em']->getFilters()->getFilter('soft_delete');
        $softDeleteFilter->setExcludes(array(
        ...
        $app['eccube.event.dispatcher']->dispatch(EccubeEvents::FRONT_MYPAGE_MYPAGE_VIDEO_INITIALIZE, $event);
